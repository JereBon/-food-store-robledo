...
| **us-010-bugfixes** | Post-entrega (Auditoría) | Corrección de imports rotos en tests backend, `__init__.py` en dirs de tests, reescritura M2M de test_product_association, fix test_router con UoW parcheado, migración Pydantic v2 (`class Config` → `ConfigDict`), mocks de tests frontend alineados con `useCategoriesPaginated`, docs API_CATEGORIES en español con URL correcta, `.gitignore` para logs. 112/112 backend + 106/106 frontend. | ✅ Completado |