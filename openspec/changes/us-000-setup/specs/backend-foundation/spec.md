...
- **THEN** existen módulos para `auth`, `usuarios`, `productos`, `categorias`, `pedidos`, `pagos`, `direcciones`, `admin` y `refreshtokens` con archivos placeholder de sus capas
...
El backend SHALL proveer un `BaseRepository[T]` genérico con operaciones CRUD comunes, para minimizar duplicación y estandarizar acceso a datos.
