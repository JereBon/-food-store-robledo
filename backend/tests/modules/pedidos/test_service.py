...
def _make_service(*, stock=10, price=Decimal("100.00"), has_address=True):
    ...
    return PedidoService(pedido_repo, direccion_repo, producto_repo)


def test_create_order_success():
    service = _make_service(stock=5, price=Decimal("250.00"))
    data = PedidoCreate(direccion_id=1, items=[ItemCreate(producto_id=1, cantidad=2, exclusiones=[])])
...
def test_create_order_insufficient_stock_raises():
    ...
    data = PedidoCreate(direccion_id=1, items=[ItemCreate(producto_id=1, cantidad=3)])
...
def test_create_order_missing_address_raises():
    ...
    data = PedidoCreate(direccion_id=99, items=[ItemCreate(producto_id=1, cantidad=1)])
...
def test_create_order_stores_price_snapshot():
    ...
    data = PedidoCreate(
        direccion_id=1,
        items=[
            ItemCreate(producto_id=1, cantidad=2),
            ItemCreate(producto_id=1, cantidad=1),
        ],
    )
    pedido = service.create_order(usuario_id=1, data=data)
    # 2 * 350 + 1 * 350 = 1050
...
def test_create_order_calls_historial():
    ...
    data = PedidoCreate(direccion_id=1, items=[ItemCreate(producto_id=1, cantidad=1)])
    service.create_order(usuario_id=1, data=data)
